...
(function(clientubb) {
	...
	clientubb.searchubb = function(tagid, posttype, thetdclass, myUBBCode, target, canimgsg, canimgcd, canmdcd, canemcd) {
		canimgsign = canimgsg;
		canimgcode = canimgcd;
		canmediacode = canmdcd;
		canemcode = canemcd;

		tdclass = thetdclass;
		// var UbbInnerHtml = $(tagid).innerHTML.replace(/\<br\>/g, "<BR>").replace(/\r\n/g, '');
		var UbbInnerHtml = myUBBCode.replace(/\n/g, "<BR>");;
		if (posttype == 3)
		// $(tagid).innerHTML = ubb.noubb(ubbsigncode(UbbInnerHtml));
			target.innerHTML = ubb.noubb(ubbsigncode(UbbInnerHtml));
		else {
			...
			target.innerHTML = ubb.noubb(ubb.code(ubb.noubb(currubb), '', 1));
		}
	}
	...
	function ubbcode(str) {
		...
		while (pattern.test(str)) {
			...
			ubb.storage.code.push(ubb.color(ubb.code(codeSource, language, 0)));
		...
		if ($('showAllImages')) {
			$('showAllImages').addEvent('click', function() {
				$$('a.clickloadImage').each(function(item) {
					(function() {
						item.innerHTML = loadImg(item.href);
					}).delay(100);
				})
			})
		}
	...
	var codeUbb = function(source, language) {
		...
		for (loop = 0; loop < source.length; loop++) {
			...
			if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c == '_') {
				...
				if (temp.length > 0) {
					if (ubb.keywords.indexOf(temp) != -1) {
				...
				if (c == '/') {
					...
					for (;
						(source.charAt(r - 1) == '\\') && (r != -1) && (source.charAt(r - 2) != '\\');) {
...
}(window.clientubb = window.clientubb || {}));